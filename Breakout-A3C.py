...
# This is A3C(Asynchronous Advantage Actor Critic) agent
...
class A3CAgent:
    ...
    def train(self):
        ...
        agents = [Agent(self.action_size, self.state_size, [self.actor, self.critic], self.sess, self.optimizer, self.discount_factor, [self.summary_op, self.summary_placeholders, self.update_ops, self.summary_writer]) for _ in range(self.threads)]
        ...
        while True:
            time.sleep(60*10)
            self.save_model("breakout_a3c/save_model/breakout_a3c")
    ...
    def build_model(self):
        ...
        policy = Dense(self.action_size, activation='softmax')(fc)  # 离散策略 softmax
        value = Dense(1, activation='linear')(fc)                   # 状态值函数
...
class Agent(threading.Thread):
    def __init__(self, action_size, state_size, model, sess, optimizer, discount_factor, summary_ops):
        ...
        # local 的 actor 和 critic 用于产生 on-policy 的样本，定期更新参数
    ...
    def run(self):
        ...
        while episode < EPISODES:
            ...
            while not done:
                ...
                # real_action 执行的动作是由 local_actor 产生的，不是全局actor
                next_observe, reward, done, info = env.step(real_action)

                # next_state 添加到 history中，history维护最近的四个状态的连接
                ...
                # 如果大于当前存储的样本容量 > 20，则取出计算Actor和Critic的梯度
                if self.t >= self.t_max or done:
                    self.train_model(done)
                    self.update_localmodel()   # 从全局模型获取最新的参数
                    self.t = 0

                # if done, plot the sqcore over episodes
                if done:
                    episode += 1
    ...
    def discount_rewards(self, rewards, done):
        """
            参数rewards是一个周期内产生的所有立即奖励，用该函数转换为 n-step return
        """
        discounted_rewards = np.zeros_like(rewards)
        running_add = 0
        if not done:       # 周期内最后一个状态的值函数
    ...
    def train_model(self, done):
        """
            计算 advantage function，进而构造 Actor 和 Critic 的损失
        """
        ...
        values = self.critic.predict(states)     # 直接改变全局critic的参数
    ...
    def get_action(self, history):
        """
            根据 local model 采样。注意不能根据全局model
        """
    ...
    def memory(self, history, action, reward):
        ...
        act = np.zeros(self.action_size)    # one-hot action
        act[action] = 1
